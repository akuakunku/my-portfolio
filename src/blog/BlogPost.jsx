      ...
      <header className="mb-12">
        ...
        >
          <div className="text-sm text-gray-600 dark:text-gray-400 mb-2">
            <span className="font-semibold">{post.author}</span> | {new Date(post.created_at).toLocaleDateString()}
          </div>
          <h1 className="text-4xl font-extrabold text-gray-900 dark:text-yellow-500 mb-4">{post.title}</h1>
          <div className="border-t border-gray-300 dark:border-gray-700 mb-6"></div> {/* Horizontal line above content */}
          {post.image_url && (
            <motion.img
              src={post.image_url}
              alt={post.title}
